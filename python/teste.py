...
def update_key(key, should_press):
    """Atualiza o estado de uma tecla, pressionando ou soltando."""
    global key_states
    if should_press and not key_states[key]:
        keyboard_controller.press(key)
        key_states[key] = True
    elif not should_press and key_states[key]:
        keyboard_controller.release(key)
        key_states[key] = False

def process_input(axis, value):
    """Processa os valores do analógico e simula pressionamentos de teclas."""
    if axis == 0:  
        if value > 30:
            update_key('d', True)
            update_key('a', False)
...
def controle(ser):
    ...
    try:
        ...
        # Solta todas as teclas ao interromper
        for key in key_states:
            if key_states[key]:
                keyboard_controller.release(key)
        raise  # Re-levanta para ser tratado no nível superior
